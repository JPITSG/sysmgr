...
- **Remote notifications & high-priority alerts** — the server can push a
  ...
  High-priority alerts and remote `action=alarm` commands stop on volume up/down,
...
- **Remote Link quality tests** — measure average round-trip latency over a
  10-second sample window with live results, then measure both directions with
